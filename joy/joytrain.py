...
import joydriving_data as driving_data
...
loss_val = tf.reduce_mean(tf.square(tf.subtract(model.y_, model.y))) + tf.add_n([tf.nn.l2_loss(v) for v in train_vars]) * L2NormConst

###print('model.y_:', model.y_.shape)

###print('model.y_:', tf.reshape(model.y_, [-1]).shape)

######onehot_labels = tf.one_hot(indices=tf.reshape(tf.cast(model.y_, tf.int32),[-1]), depth=4)

#print('onehot:', tf.reshape(onehot_labels,[4]).shape, 'y:', model.y.shape)
...
print("Run the command line:\n" \
          "--> tensorboard --logdir=./joylogs --port=6006" \
